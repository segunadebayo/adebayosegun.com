...
Another common request we get is to add more complex components, like a date picker, custom select, nested menus, etc. Today, we have a lot React hooks that can be leveraged but we've found that the complexity or cognitive overhead with understand the logic of a component works increases significantly with new features or patches.

Designing an API for these components that is easy to maintain, extend and feels intuitive is very challenging.
